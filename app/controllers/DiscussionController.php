...
class DiscussionController extends Controller {
    ...
    public function store() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($data['title_err']) && empty($data['description_err']) && empty($data['participants_err'])) {
                // Jika user adalah leader, periksa apakah ada admin dalam daftar peserta
                if ($_SESSION['role'] == 'leader') {
                    $adminFound = false;
                    foreach ($participants as $key => $participant_id) {
                        $user = $this->userModel->getUserById($participant_id);
                        if ($user && $user['role'] == 'admin') {
                            // Hapus admin dari daftar peserta
                            unset($participants[$key]);
                            $adminFound = true;
                        }
                    }
                    
                    // Jika ditemukan admin, tampilkan pesan error
                    if ($adminFound) {
                        $_SESSION['error_message'] = 'Leader tidak diizinkan menambahkan admin ke diskusi';
                    }
                    
                    // Jika tidak ada peserta yang tersisa setelah menghapus admin
                    if (empty($participants) && $adminFound) {
                        $data['participants_err'] = 'Silakan pilih minimal satu peserta (non-admin)';
                        $this->view('discussions/create', $data);
                        return;
                    }
                }
    ...
    public function addMember() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            // Cek role pengguna yang akan ditambahkan
            $userToAdd = $this->userModel->getUserById($userId);
            
            // Jika leader mencoba menambahkan admin, tolak permintaan
            if ($_SESSION['role'] == 'leader' && $userToAdd['role'] == 'admin') {
                $_SESSION['error_message'] = 'Leader tidak diizinkan menambahkan admin ke diskusi';
