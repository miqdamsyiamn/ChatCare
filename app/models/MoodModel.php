...
class MoodModel
{
    ...
    /**
     * Convert datetime string to ISO 8601 in Asia/Jakarta timezone
     * @param string $datetime UTC or any timezone
     * @return string ISO 8601 with +07:00 offset
     */
    private function toJakartaIso($datetime)
    {
        $dt = new DateTime($datetime, new DateTimeZone('UTC'));
        $dt->setTimezone(new DateTimeZone('Asia/Jakarta'));
        return $dt->format('Y-m-d\TH:i:sP'); // ISO 8601 with offset
    }
    ...
    public function getMoodData($sessionId)
    {
        ...
        foreach ($messages as $message) {
            ...
            // Add data point with Jakarta ISO timestamp
            $jakartaIso = $this->toJakartaIso($message['timestamp']);
            $data['labels'][] = $jakartaIso;
            $data['datasets'][0]['data'][] = ['x' => $jakartaIso, 'y' => $positifCount];
            $data['datasets'][1]['data'][] = ['x' => $jakartaIso, 'y' => $netralCount];
            $data['datasets'][2]['data'][] = ['x' => $jakartaIso, 'y' => $negatifCount];
        }
