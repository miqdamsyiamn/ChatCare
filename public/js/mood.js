...
document.addEventListener("DOMContentLoaded", function () {
  ...
  try {
    moodData = JSON.parse(
      document.getElementById("mood-data").getAttribute("data-mood")
    );
  } catch (error) {
    console.error("Error parsing mood data:", error);
    return;
  }
  // No need to convert moodData.labels or dataset x values to local time here, backend already sends ISO 8601 Asia/Jakarta

  // Create line chart for emotion trends
  const lineChart = new Chart(lineChartElement.getContext("2d"), {
    type: "line",
    data: {
      labels: moodData.labels || [], // pastikan labels ada
      datasets: (moodData.datasets || []).map((ds) => ({
        ...ds,
        data: (ds.data || []).map((point) => {
          // Pastikan x adalah ISO string/Date agar Chart.js time scale bisa parsing
          if (typeof point === "object" && point.x) {
            // Jika sudah Date, biarkan. Jika string, biarkan. Jika bukan, konversi.
            return {
              ...point,
              x: point.x instanceof Date ? point.x : String(point.x),
            };
          }
          return point;
        }),
    ...
    options: {
      ...
      scales: {
        ...
        x: {
          type: "time",
          time: {
            parser: true, // auto-detect ISO 8601
          ...
          ticks: {
            callback: function (value, index, ticks) {
              // Ambil nilai waktu asli dari ticks[index].value jika ada, agar tidak dapat string hasil format
              const rawValue = ticks?.[index]?.value ?? value;
  ...
  function updateCharts(newMoodData) {
    ...
    (newMoodData.datasets || []).forEach((ds) => {
      if (Array.isArray(ds.data)) {
        ds.data = ds.data.map((point) => {
          if (typeof point === "object" && point.x) {
            return { ...point, x: toJakartaISOString(point.x) };
          }
          return point;
        });
      }
  ...
  function updateMoodSummary(newMoodData) {
    ...
    const positifPercent =
      total > 0 ? Math.round((positifCount / total) * 100) : 0;
    const netralPercent =
      total > 0 ? Math.round((netralCount / total) * 100) : 0;
    const negatifPercent =
      total > 0 ? Math.round((negatifCount / total) * 100) : 0;
    ...
    if (positifProgressElement)
      positifProgressElement.style.width = `${positifPercent}%`;
    if (netralProgressElement)
      netralProgressElement.style.width = `${netralPercent}%`;
    if (negatifProgressElement)
      negatifProgressElement.style.width = `${negatifPercent}%`;
  ...
  const isSessionActive =
    document.getElementById("session-status").getAttribute("data-active") ===
    "true";
  ...
  function updateUserMoodData(userMoodData) {
    ...
    if (
      userPositifCount > userNegatifCount &&
      userPositifCount > userNetralCount
    ) {
      userMoodText = "Positif";
      userMoodColor = "bg-green-100 text-green-800";
    } else if (
      userNegatifCount > userPositifCount &&
      userNegatifCount > userNetralCount
    ) {
    ...
    const userPositifCountElement = document.querySelector(
      ".user-positif-count"
    );
    const userNetralCountElement = document.querySelector(".user-netral-count");
    const userNegatifCountElement = document.querySelector(
      ".user-negatif-count"
    );

    if (userPositifCountElement)
      userPositifCountElement.textContent = userPositifCount;
    if (userNetralCountElement)
      userNetralCountElement.textContent = userNetralCount;
    if (userNegatifCountElement)
      userNegatifCountElement.textContent = userNegatifCount;
  }
});
